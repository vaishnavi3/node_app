                    ...
                    _data.delete('checks', checkId,function(err){
                      ...
                      if(checksDeleted == checksToDelete){
                        if(!deletionErrors){
                          ...
                          callback(500, {'Error': 'There were errors while attempting to delete all of use=rs checks.All checks may not have been deleted'})
                        }
                      }
                    })
                  })
...
handlers._checks.post = function(data, callback){
  ...
  var successCodes = typeof(data.payload.successCodes) =='object' && data.payload.successCodes.length > 0 && data.payload.successCodes instanceof Array ? data.payload.successCodes : false
        ...
        _data.read('users', userPhone, function(err, userData){
          if(!err && userData){
            ...
            if(userChecks.length< config.maxChecks){
              ...
              callback(400, {'Error': 'User also has max no of checks('+config.maxChecks+')'})
            }
          }else{
            callback(403)
          }
...
handlers._checks.put = function(data, callback){
  ...
  if(id){
    if(protocol || url || method || successCodes || timeoutSeconds ){
      ...
      callback(400, {'Error': 'Missing fields to update checks'})
    }
  }else{
...
// check this logic, user is updated  to undefined, throws err ---------------
  ...
  if (id){
    _data.read('checks',id, function(err, checkData){
      if(!err && checkData){
                ...
                _data.read('users', checkData.userPhone, function(err, userData){
                  if(!err && userData){
                    ...
                    if(checkPosition > -1){
                      ...
                      _data.update('users', checkData.userPhone, function(err){
                        if(!err){
                          callback(200)
                        }else{
                      ...
                      callback(500, {'Error': 'Could not find check on users object, so could not remove it'})
                    }
                  }else{
        ...
        callback(400,{'Erro': 'Specified check id does not exist'})
      }
    })
  }else{
    callback(400, {'Error': 'Required field missing'})
